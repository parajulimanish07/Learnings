...
def main():
    ...
    while is_running:
        ...
        print(f"Guess: {guess}")  # Debug print
        ...
        if guess in answer:
            for i in range(len(answer)): #replace the underscore in the hint array with the correct letter
                if answer[i] == guess:
                    ...
                    print(f"Correct guess: {guess} at position {i}")  # Debug print
        else:
            wrong_guesses += 1
            print(f"Wrong guess: {guess}. Total wrong guesses: {wrong_guesses}")  # Debug print
...
if __name__ == "__main__": #run the program only if it's being run directly
    main()